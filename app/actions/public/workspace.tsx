...
export function WorkspaceContent(
  ...
  function currentSelection() {
    return handle.props.example ? new Set(handle.props.example.selected) : selected
  }
  function updateSelection(value: Set<number>) {
    selected = value
    handle.props.example?.onSelection([...value])
    handle.update()
  }
        ...
        <section class="activities" aria-labelledby="activities-title">
          ...
          >
            ...
            <div class="ride-list">
              {visible.map((r) => {
                ...
                return (
                  <label
                    class={
                      'ride-row' + (active ? ' selected' : '') + (unavailable ? ' unavailable' : '')
                    }
                    key={r.id}
                  >
                    <input
              ...
              {!visible.length && (
                <div class="list-empty">
                  <h3>{activities.length ? 'No matching activities' : 'No activities here yet'}</h3>
                  <p>
                    {activities.length
            ...
            <div class="selection-footer">
              ...
              ) : handle.props.example ? (
                <button
                  class="button button-dark wide"
                  data-funnel="example_review_click"
                  data-funnel-placement="example"
                  type="button"
                  disabled={selected.size < 2}
                  mix={on('click', (event) => handle.props.example?.onPreview(event))}
                >
                  Review this join <span aria-hidden="true">→</span>
                </button>
              ) : (
                <a
...
export const Workspace = clientEntry(
  ...
  function Workspace(handle: Handle<WorkspaceProps>) {
    return () => <WorkspaceContent {...handle.props} />
  },
)
